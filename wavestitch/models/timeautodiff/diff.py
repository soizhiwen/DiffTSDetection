...
class BiRNN_score(nn.Module):
    def __init__(
        ...
        #     4 * hidden_size, [], hidden_size, final_activation=nn.ReLU()
        # )
        self.proj = FeedForward(
            3 * hidden_size, [], hidden_size, final_activation=nn.ReLU()
        )
        ...
        self.cond_GRU = nn.GRU(
            emb_dim, hidden_size, num_layers, batch_first=True, bidirectional=True
        )
        ...
        if cond is not None:
            ...
            if Embedding.shape[1] != shape[1]:
                padding_tensor = torch.zeros(
                    shape[0], shape[1] - Embedding.shape[1], Embedding.shape[2]
                ).to(device)
                Embedding = torch.cat((Embedding, padding_tensor), dim=1)
            cond_out, _ = self.cond_GRU(Embedding)
            # x = self.proj(
            #     torch.cat([x + self.cond_output(cond_out), t, i, time_info], -1)
            # )
            x = self.proj(
                torch.cat([x + self.cond_output(cond_out), t, i], -1)
            )
        else:
