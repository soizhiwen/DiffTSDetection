...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        for batch in dataloader:
            ...
            conditional_mask = torch.zeros_like(batch).to(device)
            conditional_signal = torch.zeros_like(batch).to(device)
            sigmas_predicted = model((batch_noised, conditional_signal, conditional_mask, timesteps)).permute((0, 2, 1))
            optimizer.zero_grad()
