...
class Diffusion_TS(nn.Module):
    ...
    @torch.no_grad()
    def inverse_fast_sample(self, img, sampling_timesteps, clip_denoised=False):
    ...
    @torch.no_grad()
    def fast_sample(self, img, sampling_timesteps, clip_denoised=False):
        batch, device, total_timesteps, eta = (
            img.shape[0],
            self.betas.device,
            self.num_timesteps,
    ...
    def step_denoise(self, x_start, noise=None, clip_denoised=False):
        batch, device = x_start.shape[0], self.betas.device
        noise = default(noise, lambda: torch.normal(0, 1, size=x_start.shape))
        noise = noise.to(device)
        t = torch.randint(1, self.num_timesteps, (batch,), device=device).long()
        x = self.q_sample(x_start=x_start, t=t, noise=noise)  # noise sample
        pred_noise, _ = self.model_predictions(x, t, clip_x_start=clip_denoised)
        loss = F.mse_loss(pred_noise, noise, reduction="none").mean(dim=(1, 2))
        return loss
